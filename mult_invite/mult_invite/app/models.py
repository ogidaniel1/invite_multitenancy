...
def create_default_admin():
    ...
    if not default_admin:
        ...
        # is_admin=True, # No longer needed if 'role' is the primary source of truth
...
def admin_or_super_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not current_user.is_authenticated:
            ...
            # Redirect to the general universal login route
            return redirect(url_for('inv.universal_login')) # Use your blueprint name 'app_'

        # 2. Check if the authenticated user has any of the required admin roles
        #    Make sure your Admin model has these properties (is_super_admin, is_org_admin, is_location_admin)
        if not (current_user.is_super_admin or current_user.is_org_admin or current_user.is_location_admin):
            ...
            # Redirect to a safe page, like the universal login or a general dashboard
            return redirect(url_for('inv.universal_login')) # Or url_for('app_.dashboard') if they can see something
